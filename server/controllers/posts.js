...
//   const Post = req.body;
//   const newContractPost = new ContractOverview(Post);
//   console.log("Post at server ", Post);

//   try {
//     await newContractPost.save();
//     res.status(201).json(newContractPost);
//   } catch (error) {
//     res.status(409).json({ message: error.message });
//   }
// };
...
export const createContractPost = async (req, res) => {
  ...
  try {
    const savedContract = await newContractPost.save();
    return res.status(201).json(savedContract);
  } catch (error) {
    console.error("Error saving contract:", error.message);
    return res.status(409).json({ message: error.message }); // Return the error message to the client
  }
};
